  ...
  : nh_(n), has_active_goal_(false)
{
  ...
  action_server_gripper_command_.reset(
      new GCAS(nh_, address, boost::bind(&GripperCommandActionController::goalCBFollow, this, _1),
               boost::bind(&GripperCommandActionController::cancelCBFollow, this, _1), false));
  ...
  while (ros::ok() && !last_finger_state_)
  {
    ...
    {
      ROS_WARN("Waited for the kinova driver for 30 seconds, but it never showed up. Continue waiting the finger "
               "state");
...
void GripperCommandActionController::goalCBFollow(GCAS::GoalHandle gh)
{
  ...
  // gripper_gap 0 is open, 1.4 is close. Here command.position from Moveit.rviz is finger radian value, rather than
  // Cartesian gap of gripper in meter.
...
{
  ROS_INFO_ONCE("Gripper_command__action_server receive feedback of trajectory state from topic: "
  ...
  if (abs_error1 < gripper_command_goal_constraint_ && abs_error2 < gripper_command_goal_constraint_ &&
      abs_error3 < gripper_command_goal_constraint_)
  {
