...
GOAL_EPSILON = 0.05 #if all joints are less than epsilon at goal, then complete, goal is achieved. Update as you wish depending on the compromise between accuracy and speed you want to achieve
START_EPSILON = 1 #if all joints are less than epsilon at start, the trajectory can begin (prevents jumping)
START_MAX_DIST = 1 #if ANY joint is greater than start max distance, throw an error
...
class PIDController(object):
    ...
    def joint_angles_callback(self, msg):
        ...
        self._curr_pos_pub = rospy.Publisher('/pid/curr_pos',
                                       std_msgs.msg.Float32MultiArray,
                                       queue_size=1)
        pid_curr_pos = Float32MultiArray()
        pid_curr_pos.data = curr_pos
        self._curr_pos_pub.publish(pid_curr_pos)
    ...
    def update_target_pos(self, curr_pos):
        ...
        """
        self._target_pos_pub = rospy.Publisher('/pid/target_pos',
                                       std_msgs.msg.Float32MultiArray,
                                       queue_size=1)
        pid_target_pos = Float32MultiArray()
        ...
        if not self.reached_start:
            ...
            if is_at_start:
                ...
                pid_target_pos.data = self.target_pos
                self._target_pos_pub.publish(pid_target_pos)
